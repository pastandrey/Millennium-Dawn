...
for path in sorted(dirpath.glob('*.md')):
    ...
    header_lines.append('')
    header_lines.append(f"![Flag_of_{country.replace(' ', '_')}]({{ '/uploads/flags/{flag_name}.png' | relative_url }}){{: .country-flag}}")
    header_lines.append('')
    header = "\n".join(header_lines)
    ...
    if i < len(lines) and re.match(r'##\s+', lines[i]):
        ...
        if i < len(lines) and lines[i].strip() == '':
            i += 1
    # skip image line
    if i < len(lines) and lines[i].strip().startswith('![') and '.country-flag' in lines[i]:
        i += 1
        if i < len(lines) and lines[i].strip() == '':
            i += 1
    newbody = header + '\n'.join(lines[i:]).lstrip('\n')
