(function (global) {
    ...
    MDToc.ensureHeadingIds = function ensureHeadingIds(headings) {
        headings.forEach(function (h, i) {
            if (!h.id) h.id = 'heading-' + i;
        });
    };

    MDToc.buildTree = function buildTree(headings) {
        var tree = [];
        var currentH2 = null;
        var currentH3 = null;
    ...
    MDToc.renderNav = function renderNav(nav, tree) {
        var sectionIdx = 0;
    ...
    MDToc.toggleSublist = function toggleSublist(btn, sublist, open) {
        if (!sublist) return;
    ...
    MDToc.bindExpandButtons = function bindExpandButtons(nav) {
        nav.querySelectorAll('.toc-sidebar__expand').forEach(function (btn) {
            btn.addEventListener('click', function (e) {
                ...
                MDToc.toggleSublist(btn, sublist, !isOpen);
            });
        });
    };
})(window);
